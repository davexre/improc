...
public class DiagonalMatrix {
	 ...
	 * elements to 0.
	 */
	public DiagonalMatrix(int aSizeM) {
		resize(aSizeM);
	}
	...
	public boolean equals(Object obj) {
		if (obj == null || !(obj instanceof DiagonalMatrix))
			return false;
		if (obj == this)
			return true;
		DiagonalMatrix a = (DiagonalMatrix) obj;
		if (a.sizeM != sizeM)
			return false;
	...
	public String toString() {
		...
		for (int j = 0; j < sizeM; j++) {
			for (int i = 0; i <= j; i++) {
				if (i != 0)
					result.append(" ");
				result.append(String.format("%1$5.10f", new Object[] { new Double(getItem(i, j)) } ));
			}
			result.append("\n");
		}
	...
	/**
	 * Returns the size of the DiagonalMatrix as a vector
	 * 
	 * @see DiagonalMatrix#getVectorItem(int)
	 */
	 ...
	 * <i>Matrix</i> not <i>DiagonalMatrix</i></b>
	 */
	 ...
	 * <i>DiagonalMatrix</i></b>. If normalization is not possible, i.e.
	 * sumAll() returns 0, the DiagonalMatrix.make0() is called. The formula
	 * is:<br>
	 ...
	 * DiagonalMatrix.make0() is called. The formula is:<br>
	...
	/**
	 * Returns a new symetrical matrix of class Matrix (not DiagonalMatrix).
	 * 
	...
	/**
	 * Copies this DiagonalMatrix to the dest Matrix. The dest becomes a symetrical matrix.
	 ...
	 */
	public DiagonalMatrix makeCopy() {
		DiagonalMatrix result = new DiagonalMatrix(sizeM);
		copyTo(result);
		return result;
	}
	 ...
	 * necessary.
	 */
	public void copyTo(DiagonalMatrix dest) {
		dest.resize(sizeM);
		for (int i = m.length - 1; i >= 0; i--)
			dest.m[i] = m[i];
	}
	 ...
	 * <b>Warning:</b>Do mind that the matrix is DiagonalMatrix and
	...
	public static DiagonalMatrix fromXML(Element source) throws JDOMException {
		...
		DiagonalMatrix r = new DiagonalMatrix(size);
		r.make0();
