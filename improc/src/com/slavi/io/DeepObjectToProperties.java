...
public class DeepObjectToProperties {
	...
	public Object propertiesToObject(Properties properties, String prefix, boolean setToNullMissingProperties) throws IntrospectionException, IllegalArgumentException, SecurityException, InstantiationException, IllegalAccessException, InvocationTargetException, NoSuchMethodException, ClassNotFoundException {
	...
	private void internalPropertiesToObject(Properties properties, String prefix, Object object, boolean setToNullMissingProperties) throws IntrospectionException, IllegalArgumentException, SecurityException, InstantiationException, IllegalAccessException, InvocationTargetException, NoSuchMethodException, ClassNotFoundException {
		...
		while (currClass != null) {
			...
			for (Field field : fields) {
				...
				if (Modifier.isStatic(modifiers))
					continue;
				String fieldPrefix = getChildPrefix(prefix, field.getName());
				...
				if (Modifier.isFinal(modifiers)) {
					...
					if (value == null)
						continue;
					readObjectIds.add(value);
					System.out.println("read add (1) :" + value.getClass() + " " + value.hashCode() + " " + prefix);
					if (fieldType.getComponentType() != null) {
						// array
						String fieldClassName = "[" + fieldType.getComponentType().getName() + "]";
						Object value2 = propertiesToObject(properties, fieldPrefix, fieldClassName, setToNullMissingProperties);
						...
						internalPropertiesToObject(properties, fieldPrefix, value, setToNullMissingProperties);
					}
					continue;
				}
				...
				if (Modifier.isFinal(fieldType.getModifiers())) {
					String classStr = properties.getProperty(getChildPrefix(fieldPrefix, "$class"));
					if (classStr != null && (!classStr.startsWith("$ref$")))
						fieldItemNeedClassTag = false;
				}
				
				Object value;
				if (fieldItemNeedClassTag)
					value = propertiesToObject(properties, fieldPrefix, setToNullMissingProperties);
				else {
					...
					value = propertiesToObject(properties, fieldPrefix, fieldClassName, setToNullMissingProperties);
				}
	...
	private Object propertiesToObject(Properties properties, String prefix, String objectClassName, boolean setToNullMissingProperties) throws IntrospectionException, IllegalArgumentException, SecurityException, InstantiationException, IllegalAccessException, InvocationTargetException, NoSuchMethodException, ClassNotFoundException {
		...
		if (objectClassName.startsWith("[") && objectClassName.endsWith("]")) {
			...
			System.out.println("read add (2) :" + array.getClass() + " " + array.hashCode() + " " + prefix);
		...
		System.out.println("read add (3) :" + object.getClass() + " " + object.hashCode() + " " + prefix);
	...
	public void objectToProperties(Properties properties, String prefix, Object object) throws IntrospectionException, IllegalArgumentException, IllegalAccessException, InvocationTargetException {
		objectToProperties(properties, prefix, object, true);
	}
	
	private void objectToProperties(Properties properties, String prefix, Object object, boolean needsClassTag) throws IntrospectionException, IllegalArgumentException, IllegalAccessException, InvocationTargetException {
		System.out.println("write prefix=" + prefix);
		if (object == null)
			return;
		Integer objectId = writeObjectIds.get(object);
		if (objectId != null) {
			properties.setProperty(getChildPrefix(prefix, "$class"), "$ref$" + objectId);
			return;
		}
		...
		if (primitiveClasses.containsKey(objectClass.getName()) || objectClass.isEnum()) {
			...
			System.out.println("write add (1) :" + object.getClass() + " " + object.hashCode() + " " + prefix);
			// array
			...
			writeObjectIds.put(object, writeObjectIds.size() + 1);
			System.out.println("write add (2) :" + object.getClass() + " " + object.hashCode() + " " + prefix);
			if (needsClassTag)
				properties.setProperty(getChildPrefix(prefix, "$class"), objectClass.getName());
