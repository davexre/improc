...
public class DWindowedImageUtils {
	...
	public static void drawBorder(Rectangle innerRect, DWindowedImage dest, double borderColor) {
		int dMinX = dest.minX();
		int dMaxX = dest.maxX();
		int dMinY = dest.minY();
		int dMaxY = dest.maxY();
	...
	public static void copyTo(DWindowedImage source, DWindowedImage dest) {
		...
		drawBorder(source.getExtent(), dest, 0.0);
		for (int i = minX; i <= maxX; i++)
			for (int j = minY; j <= maxY; j++) 
