...
public class CalculatePanoramaParams2 implements Callable<Void> {
	...
	public CalculatePanoramaParams2(ExecutorService exec,
			ArrayList<KeyPointPairList> kppl,
			AbsoluteToRelativePathMaker keyPointPairFileRoot,
			String outputDir,
			boolean pinPoints,
	...
	private class ProcessOne implements Callable<Void> {
		...
		public Void call() throws Exception {
			while (true) {
				...
				while (true) {
					...
					double maxDiscrepnacy = computeDiscrepancies();
					boolean isDone = false;
					if (maxDiscrepnacy > discrepancyThreshold) {
						if (recomputeBad()) {
							isDone = true;
						}
					} else {
		...
		protected double computeDiscrepancies() {
			...
			for (KeyPointPairList pairList : chain) {
				...
				if (maxMaxDiscrepancy < pairList.maxDiscrepancy)
					maxMaxDiscrepancy = pairList.maxDiscrepancy;
			}
			return maxMaxDiscrepancy;
		}
		
		void calculateNormalEquations() {
			Matrix coefs = new Matrix(images.size(), 1);			
