...
public class DGaussianFilter {
	...
	public void applyGaussianFilter(DImageMap src, DImageMap dest) {
		dest.resize(src.getSizeX(), src.getSizeY());

		if ((bufY == null) || (bufY.length != src.getSizeY() + mask.length - 1))
			bufY = new double[src.getSizeY() + mask.length - 1];
		for (int i = src.getSizeX() - 1; i >= 0; i--) {
			...
			for (int j = maskRadius - 1; j > 0; j--)
				bufY[bufYIndx++] = tmp;
			for (int j = 0; j < src.getSizeY(); j++)
				bufY[bufYIndx++] = src.getPixel(i, j);
			tmp = src.getPixel(i, src.getSizeY() - 1);
			for (int j = maskRadius - 1; j > 0; j--)
				bufY[bufYIndx++] = tmp;
		...
		if ((bufX == null) || (bufX.length != src.getSizeX() + mask.length - 1))
			bufX = new double[src.getSizeX() + mask.length - 1];
		for (int j = src.getSizeY() - 1; j >= 0; j--) {
			// fill in the buffer
			int bufXIndx = 0;
			...
			tmp = dest.getPixel(src.getSizeX() - 1, j);
			for (int i = maskRadius - 1; i > 0; i--)
				bufX[bufXIndx++] = tmp;
	...
	public void applyGaussianFilterOriginal(DImageMap src, DImageMap dest) {
		DImageMap tmp = new DImageMap(src.getSizeX(), src.getSizeY());
		dest.resize(src.getSizeX(), src.getSizeY());
