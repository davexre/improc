...
public class PanoList {
	...
	public static int writeToPtoFile(File ptoFile, ArrayList<PanoPairList>chain) throws FileNotFoundException {
		if (chain == null)
			return 0;
		if (chain.size() < 1)
			return 0;
		...
		PrintStream fou2 = new PrintStream(ptoFile.getAbsolutePath() + ".txt");
		...
		for (PanoPairList p : chain) {
			...
			for (PanoPair pp : p.items) {
				...
				fou2.println(pp.discrepancy + "\t" + pp.distance1 + "\t" + pp.distance2 + "\t" + (pp.distance1 / pp.distance2));
			}
		}		
