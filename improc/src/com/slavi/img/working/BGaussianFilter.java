...
public class BGaussianFilter {
	 ...
	 */
	public byte[] bufX = null;

	public byte[] bufY = null;
	...
	public BGaussianFilter(double sigma) {
		this(sigma, 1 + (int) Math.floor(3.0 * sigma));
	}

	public BGaussianFilter(double sigma, int MaskRadius) {
	...
	public void applyGaussianFilter(BImageMap src, BImageMap dest) {
		dest.resize(src.sizeX, src.sizeY);

		if ((bufY == null) || (bufY.length != src.sizeY + mask.length - 1))
			bufY = new byte[src.sizeY + mask.length - 1];
		for (int i = src.sizeX - 1; i >= 0; i--) {
			// fill in the buffer
			int bufYIndx = 0;
			byte tmp = src.getPixel(i, 0);
			...
			for (int j = src.sizeY - 1; j >= 0; j--) {
				...
				for (int k = mask.length - 1; k >= 0; k--)
					sum += mask[k] * bufY[j + k];
				dest.setPixel(i, j, (byte)sum);
			}
		}

		if ((bufX == null) || (bufX.length != src.sizeX + mask.length - 1))
			bufX = new byte[src.sizeX + mask.length - 1];
		for (int j = src.sizeY - 1; j >= 0; j--) {
			// fill in the buffer
			int bufXIndx = 0;
			byte tmp = dest.getPixel(0, j);
	...
	/////////// OBSOLETE ?!?
	public void applyGaussianFilterOriginal(BImageMap src, BImageMap dest) {
		BImageMap tmp = new BImageMap(src.sizeX, src.sizeY);
		dest.resize(src.sizeX, src.sizeY);
		...
		for (int i = 0; i < src.sizeX; i++)
			for (int j = 0; j < src.sizeY; j++) {
				...
				tmp.setPixel(i, j, (byte)(sum / (1 - msum)));
			}
		...
		for (int i = 0; i < src.sizeX; i++)
			for (int j = 0; j < src.sizeY; j++) {
				...
				dest.setPixel(i, j, (byte)(sum / (1 - msum)));
			}
	}
}
