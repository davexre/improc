...
//					RotationZYZ.instance.getRotationAnglesBackword(angles[0], angles[1], angles[2], angles);
	...
	protected double computeOneDiscrepancy(KeyPointPair item, double PW1[], double PW2[]) {
		SphereNorm2.transformForeward(item.sourceSP.getDoubleX(), item.sourceSP.getDoubleY(), item.sourceSP.getKeyPointList(), PW1);
		SphereNorm2.transformForeward(item.targetSP.getDoubleX(), item.targetSP.getDoubleY(), item.targetSP.getKeyPointList(), PW2);
