...
public class MyPanoPairTransformZYXLearner extends PanoTransformer {
	...
	public void transformForeward(double sx, double sy, KeyPointList srcImage, double dest[]) {
		MyPanoPairTransformZYXNorm.transformForeward(sx, sy, srcImage, dest);
	...
	public void transformBackward(double rx, double ry, KeyPointList srcImage, double dest[]) {
		...
		MyPanoPairTransformZYXNorm.transformBackward(dest[0], dest[1], dest[2], srcImage, dest);
	}
		...
		while (curImageIndex >= 0) {
			...
			if (minHopPairList != null) {
				if (curImage == minHopPairList.source) {
					...
					Matrix sourceToTarget = MyPanoPairTransformZYXNorm.rot.makeAngles(minHopPairList.rx, minHopPairList.ry, minHopPairList.rz);
					Matrix targetToWorld = MyPanoPairTransformZYXNorm.rot.makeAngles(-minHopPairList.target.rx, -minHopPairList.target.ry, minHopPairList.target.rz);
					Matrix sourceToWorld = new Matrix(3, 3);
					sourceToTarget.mMul(targetToWorld, sourceToWorld);
					MyPanoPairTransformZYXNorm.rot.getRotationAngles(sourceToWorld, angles);
...
//					System.out.println(curImage.imageFileStamp.getFile().getName() + "\t" + minHopPairList.target.imageFileStamp.getFile().getName());
					...
					double angles[] = new double[3];
					MyPanoPairTransformZYXNorm.rot.getRotationAnglesBackword(minHopPairList.rx, minHopPairList.ry, minHopPairList.rz, angles);
					Matrix targetToSource = MyPanoPairTransformZYXNorm.rot.makeAngles(-angles[0], -angles[1], angles[2]);
					Matrix sourceToWorld = MyPanoPairTransformZYXNorm.rot.makeAngles(minHopPairList.source.rx, minHopPairList.source.ry, minHopPairList.source.rz);
					Matrix targetToWorld = new Matrix(3, 3);
					targetToSource.mMul(sourceToWorld, targetToWorld);
					MyPanoPairTransformZYXNorm.rot.getRotationAngles(targetToWorld, angles);
					curImage.rx = angles[0];
					curImage.ry = angles[1];
					curImage.rz = angles[2];
...
//		System.out.println("NORMAL EQUASIONS");
	...
	void buildCamera2RealMatrix(KeyPointList image) {
		image.camera2real = MyPanoPairTransformZYXNorm.rot.makeAngles(image.rx, image.ry, image.rz);
		image.dMdX = MyPanoPairTransformZYXNorm.rot.make_dF_dR1(image.rx, image.ry, image.rz);
		image.dMdY = MyPanoPairTransformZYXNorm.rot.make_dF_dR2(image.rx, image.ry, image.rz);
		image.dMdZ = MyPanoPairTransformZYXNorm.rot.make_dF_dR3(image.rx, image.ry, image.rz);
	}
