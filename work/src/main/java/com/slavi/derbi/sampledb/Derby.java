package com.slavi.derbi.sampledb;
...
import com.slavi.derbi.sampledb.model.OrderDetail;
...
public class Derby {
	...
	/**
	 * Generate database tables and data.
	 * <p><img src="doc-files/MySQL-Sample-Database-Schema.png" /></p>
	 */
	public static DataSource generateDb() throws Exception {
		EmbeddedDataSource ds = new EmbeddedDataSource();
		ds.setDatabaseName("memory:sampledb;create=true");
...
//		return generateDb();
	...
	public void doIt() throws Exception {
		...
		try (PreparedStatement st = conn.prepareStatement("select * from products fetch first 3 rows only")) {
			System.out.println(ResultSetToString.resultSetToString(st.executeQuery()));
		}

		TypedQuery<OrderDetail> tq = em.createQuery("select e from OrderDetail e", OrderDetail.class);
		tq.setMaxResults(1);
